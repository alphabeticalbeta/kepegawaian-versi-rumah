...
@section('title', 'Master Data Sub Unit Kerja')
        ...
        <div class="mb-8">
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-4">
                    ...
                    <div>
                        <h1 class="text-3xl font-bold text-slate-800">Master Data Sub Unit Kerja</h1>
                        <p class="text-slate-600">Kelola data Sub Unit Kerja</p>
                    </div>
                </div>
                <a href="{{ route('backend.kepegawaian-universitas.sub-unitkerja.create') }}" 
                ...
                <table class="w-full table-auto">
                    <thead class="bg-gradient-to-r from-slate-800 to-slate-900 text-white">
                        <tr>
                            ...
                            <th class="px-6 py-4 text-left font-semibold">Nama Sub Unit Kerja</th>
                            <th class="px-6 py-4 text-left font-semibold">Unit Kerja</th>
                            <th class="px-6 py-4 text-center font-semibold">Aksi</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-200">
                        @forelse($subUnitKerjas as $index => $subUnit)
                        <tr class="hover:bg-slate-50 transition-colors duration-200">
                            <td class="px-6 py-4 text-slate-600">{{ $index + 1 }}</td>
                            <td class="px-6 py-4 font-medium text-slate-800">{{ $subUnit->nama_sub_unit_kerja }}</td>
                            <td class="px-6 py-4 text-slate-600">{{ $subUnit->unitKerja->nama_unit_kerja ?? '-' }}</td>
                            <td class="px-6 py-4 text-center">
                                <div class="flex items-center justify-center gap-2">
                                    <a href="{{ route('backend.kepegawaian-universitas.sub-unitkerja.edit', $subUnit) }}" 
                                    ...
                                    <form action="{{ route('backend.kepegawaian-universitas.sub-unitkerja.destroy', $subUnit) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center text-slate-500">
                                ...
                                <p class="text-slate-400">Sub Unit Kerja belum ditambahkan</p>
                            </td>
                        </tr>
                        @endforelse
                    </tbody>
