...
echo "=== CLEANUP AND TEST FORM SUBMISSION ===\n";
...
try {
    ...
    // Cleanup existing usulan
    echo "\n2. Cleaning up existing usulan...\n";

    $existingUsulan = Usulan::where('pegawai_id', $pegawai->id)
        ->where('jenis_usulan', 'Usulan Jabatan')
        ->get();

    echo "Found " . $existingUsulan->count() . " existing usulan\n";

    foreach ($existingUsulan as $usulan) {
        // Delete related logs first
        UsulanLog::where('usulan_id', $usulan->id)->delete();
        echo "Deleted logs for usulan ID: " . $usulan->id . "\n";

        // Delete usulan
        $usulan->delete();
        echo "Deleted usulan ID: " . $usulan->id . "\n";
    }
    ...
    echo "\n4. Testing form submission for Guru Besar...\n";
