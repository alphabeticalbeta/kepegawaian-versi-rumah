...
// Script untuk generate semua controller dan view usulan pegawai
// Berdasarkan sidebar admin universitas usulan
...
$usulanTypes = [
    ...
    'presensi' => [
        ...
        'description' => 'Pengajuan Presensi'
    ],
...
echo "=== GENERATOR USULAN CONTROLLERS & VIEWS ===\n\n";

foreach ($usulanTypes as $key => $type) {
    $controllerName = 'Usulan' . str_replace('-', '', ucwords($key, '-')) . 'Controller';
    $viewPath = str_replace('-', '-', $key);

    echo "Generating for: {$type['name']}\n";
    echo "Controller: {$controllerName}\n";
    echo "View Path: usulan-{$viewPath}\n\n";
    ...
    echo "✅ Controller and View content generated\n";
    echo "📁 Files to create:\n";
    echo "   - app/Http/Controllers/Backend/PegawaiUnmul/{$controllerName}.php\n";
    echo "   - resources/views/backend/layouts/views/pegawai-unmul/usulan-{$viewPath}/index.blade.php\n\n";
}
...
echo "=== SUMMARY ===\n";
echo "Total usulan types: " . count($usulanTypes) . "\n";
echo "Controllers to create: " . count($usulanTypes) . "\n";
echo "Views to create: " . count($usulanTypes) . "\n\n";

echo "=== NEXT STEPS ===\n";
echo "1. Create all controller files\n";
echo "2. Create all view files\n";
echo "3. Add routes to web.php\n";
echo "4. Update sidebar to link to new pages\n";
echo "5. Test all pages\n\n";

echo "=== ROUTES TO ADD ===\n";
foreach ($usulanTypes as $key => $type) {
    $routePrefix = 'pegawai-unmul.usulan-' . str_replace('-', '-', $key);
    echo "Route::resource('usulan-{$key}', Usulan" . str_replace('-', '', ucwords($key, '-')) . "Controller::class)->names('{$routePrefix}');\n";
}
