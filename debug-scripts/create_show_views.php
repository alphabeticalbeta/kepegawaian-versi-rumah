...
// Daftar jenis usulan dan route names
$usulanTypes = [
    'usulan-id-sinta-sister' => 'Usulan ID SINTA ke SISTER',
...
echo "=== CREATING SHOW VIEWS FOR ALL USULAN TYPES ===\n\n";

foreach ($usulanTypes as $routeName => $displayName) {
    $viewPath = "resources/views/backend/layouts/views/pegawai-unmul/{$routeName}/show.blade.php";
    $viewDir = dirname($viewPath);

    // Create directory if not exists
    if (!is_dir($viewDir)) {
        mkdir($viewDir, 0755, true);
        echo "✅ Created directory: {$viewDir}\n";
    }
...
@section('title', 'Detail {$displayName}')
                    ...
                    @foreach(\$usulan->data_usulan['dokumen_usulan'] as \$docType => \$docData)
                        ...
                        <div class="border border-gray-200 rounded-lg p-4">
                            <div class="flex items-center justify-between">
                                ...
                                <a href="{{ route('pegawai-unmul.{$routeName}.show-document', ['{$routeName}' => \$usulan->id, 'field' => \$docType]) }}"
                                   target="_blank"
...
BLADE;
    ...
    if (file_put_contents($viewPath, $viewContent)) {
        ...
        echo "❌ Failed to create view: {$viewPath}\n";
    }
}

echo "\n=== SHOW VIEWS CREATION COMPLETED ===\n";
