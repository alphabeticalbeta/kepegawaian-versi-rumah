...
@section('title', 'Pedoman - Universitas Mulawarman')
...
<div class="min-h-screen bg-gray-50">
        ...
        <div class="relative px-6 py-4 sm:px-8 sm:py-20">
            <div class="mx-auto max-w-4xl text-center">
                ...
                <p class="text-base text-gray-600 sm:text-lg">
                    Daftar pedoman dan panduan Universitas Mulawarman
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="w-full px-2 sm:px-4 lg:px-6 py-6">
        ...
        <div class="mb-6 bg-white rounded-2xl shadow-xl p-4 transition-all duration-300 hover:shadow-2xl">
            <form method="GET" action="{{ route('dasar-hukum.pedoman.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                ...
                <div>
                    ...
                    <div class="relative">
                        ...
                        <!-- Clear button -->
                        @if(request('search'))
                            <a href="{{ route('dasar-hukum.pedoman.index') }}" class="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors" title="Hapus pencarian" id="clearSearch">
        ...
        <div class="mb-4 flex flex-wrap gap-2">
            ...
            @if(request('search'))
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-orange-100 text-orange-800">
                    ...
                    <a href="{{ route('dasar-hukum.pedoman.index', array_merge(request()->except('search'))) }}" class="ml-2 text-orange-600 hover:text-orange-800">
            ...
            @if(request('status'))
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                    ...
                    <a href="{{ route('dasar-hukum.pedoman.index', array_merge(request()->except('status'))) }}" class="ml-2 text-blue-600 hover:text-blue-800">
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </a>
            ...
            @if(request('tahun'))
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                    Tahun: {{ request('tahun') }}
                    <a href="{{ route('dasar-hukum.pedoman.index', array_merge(request()->except('tahun'))) }}" class="ml-2 text-green-600 hover:text-green-800">
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </a>
        ...
        <!-- Pedoman Grid - Hampir Penuh -->
        <div class="w-full">
            <div id="pedomanGrid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 transition-all duration-500">
                @forelse($pedoman as $index => $item)
                <article class="bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-pointer transform hover:scale-105 animate-fade-in"
                         ...
                         onclick="showPedomanDetail({{ $item->id }}, '{{ addslashes($item->judul) }}', '{{ addslashes($item->konten) }}', '{{ $item->tanggal_dokumen }}', '{{ addslashes($item->penulis ?? 'Admin') }}', '{{ addslashes($item->nomor_dokumen ?? '-') }}', '{{ addslashes($item->nama_instansi ?? '-') }}', '{{ addslashes($item->jenis_label ?? 'Pedoman') }}', '{{ addslashes($item->status_label ?? '-') }}', '{{ $item->thumbnail }}', {{ json_encode($item->lampiran ?? []) }})">

                    <!-- Thumbnail Section -->
                    <div class="relative h-48 bg-gradient-to-br from-blue-400 to-indigo-500">
                        @if($item->thumbnail)
                ...
                <div class="col-span-full text-center py-12">
                    <div class="bg-gray-50 border border-gray-200 rounded-lg p-6 max-w-md mx-auto">
                        ...
                        <h3 class="text-lg font-medium text-gray-800 mb-2">Tidak Ada Pedoman</h3>
                        <p class="text-gray-600">
                            @if(request('search') || request('status') || request('tahun'))
                                Tidak ditemukan pedoman dengan filter yang dipilih.
                            @else
                                Belum ada pedoman yang tersedia saat ini.
                            @endif
                        </p>
                    </div>
                </div>
        ...
        <div class="mt-8">
            <nav class="flex items-center justify-between">
                <div class="flex items-center text-sm text-gray-700">
                    Menampilkan {{ $pedoman->firstItem() ?? 0 }} - {{ $pedoman->lastItem() ?? 0 }} dari {{ $pedoman->total() }} pedoman
                </div>
                <div class="flex items-center space-x-2">
                    ...
                    @else
                        <a href="{{ $pedoman->previousPageUrl() }}" class="px-3 py-2 text-sm font-medium text-black bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                            </svg>
                        </a>
                    ...
                    @if($pedoman->hasMorePages())
                        <a href="{{ $pedoman->nextPageUrl() }}" class="px-3 py-2 text-sm font-medium text-black bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<!-- Pedoman Detail Modal -->
<div id="pedomanModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-hidden">
            <div class="flex items-center justify-between p-6 border-b">
                <h3 id="modalTitle" class="text-xl font-semibold text-gray-900">Detail Pedoman</h3>
...
@push('scripts')
<script>
// Show pedoman detail modal
function showPedomanDetail(id, judul, konten, tanggalDokumen, penulis, nomorDokumen, namaInstansi, jenisLabel, statusLabel, thumbnail, lampiran) {
    // Set modal title
    ...
    let modalContent = `
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-2">
                    <button onclick="sharePedoman('${escapeHtml(judul)}', '${window.location.origin}/dasar-hukum/pedoman/${id}')"
    ...
    if (lampiran && Array.isArray(lampiran) && lampiran.length > 0) {
        ...
        lampiran.forEach((file, index) => {
            ...
            modalContent += `
                <a href="/dasar-hukum/pedoman/${id}/download/${escapeHtml(filePath.split('/').pop())}"
                   target="_blank"
...
// Share functionality
function sharePedoman(title, url) {
    if (navigator.share) {
        navigator.share({
            title: title,
            url: url
...
// Print functionality
function printPedoman() {
    const modalContent = document.getElementById('modalContent').innerHTML;
    const printWindow = window.open('', '_blank');
    printWindow.document.write(`
        <html>
            <head>
                <title>Pedoman - ${document.getElementById('modalTitle').textContent}</title>
                <style>
