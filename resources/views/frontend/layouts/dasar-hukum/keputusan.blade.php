...
@section('title', 'Keputusan - Universitas Mulawarman')
...
<div class="min-h-screen bg-gray-50">
        ...
        <div class="relative px-6 py-4 sm:px-8 sm:py-20">
            <div class="mx-auto max-w-4xl text-center">
                ...
                <p class="text-base text-gray-600 sm:text-lg">
                    Daftar keputusan dan peraturan Universitas Mulawarman
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="w-full px-2 sm:px-4 lg:px-6 py-6">
        ...
        <div class="mb-6 bg-white rounded-2xl shadow-xl p-4 transition-all duration-300 hover:shadow-2xl">
            <form method="GET" action="{{ route('dasar-hukum.keputusan.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Status</label>
                    <select name="status" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all duration-200 hover:border-orange-400">
                ...
                <div>
                    ...
                    <select name="tahun" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all duration-200 hover:border-orange-400">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari judul/nomor dokumen..."
                               class="w-full px-3 py-2 pr-10 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all duration-200 hover:border-orange-400"
                               id="searchInput">
                        <!-- Loading spinner -->
                        ...
                        <!-- Clear button -->
                        @if(request('search'))
                            <a href="{{ route('dasar-hukum.keputusan.index') }}" class="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors" title="Hapus pencarian" id="clearSearch">
        ...
        <div class="mb-4 flex flex-wrap gap-2">
            ...
            @if(request('search'))
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-orange-100 text-orange-800">
                    ...
                    <a href="{{ route('dasar-hukum.keputusan.index', array_merge(request()->except('search'))) }}" class="ml-2 text-orange-600 hover:text-orange-800">
            ...
            @if(request('status'))
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                    ...
                    <a href="{{ route('dasar-hukum.keputusan.index', array_merge(request()->except('status'))) }}" class="ml-2 text-blue-600 hover:text-blue-800">
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </a>
            ...
            @if(request('tahun'))
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                    Tahun: {{ request('tahun') }}
                    <a href="{{ route('dasar-hukum.keputusan.index', array_merge(request()->except('tahun'))) }}" class="ml-2 text-green-600 hover:text-green-800">
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </a>
        ...
        <!-- Keputusan Grid - Hampir Penuh -->
        <div class="w-full">
            <div id="keputusanGrid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 transition-all duration-500">
                ...
                <article class="bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-pointer transform hover:scale-105 animate-fade-in"
                         ...
                         onclick="showKeputusanDetail({{ $item->id }}, '{{ addslashes($item->judul) }}', '{{ addslashes($item->konten) }}', '{{ $item->tanggal_dokumen }}', '{{ addslashes($item->penulis ?? 'Admin') }}', '{{ addslashes($item->nomor_dokumen ?? '-') }}', '{{ addslashes($item->nama_instansi ?? '-') }}', '{{ addslashes($item->jenis_label ?? 'Keputusan') }}', '{{ addslashes($item->status_label ?? '-') }}', '{{ $item->thumbnail }}', {{ json_encode($item->lampiran ?? []) }})">

                    <!-- Thumbnail Section -->
                    <div class="relative h-48 bg-gradient-to-br from-orange-400 to-red-500">
                        @if($item->thumbnail)
                    ...
                    <div class="p-4">
                        ...
                        <!-- Title -->
                        <h3 class="text-sm font-semibold text-gray-900 mb-2 line-clamp-2 hover:text-orange-600 transition-colors duration-300">
                            {{ $item->judul }}
                        </h3>
                        ...
                        <div class="flex items-center justify-between">
                            <div class="text-orange-600 font-medium text-xs transition-colors duration-300 hover:text-orange-800">
                                Klik untuk melihat →
                            </div>
                ...
                <div class="col-span-full text-center py-12">
                    <div class="bg-gray-50 border border-gray-200 rounded-lg p-6 max-w-md mx-auto">
                        ...
                        <h3 class="text-lg font-medium text-gray-800 mb-2">Tidak Ada Keputusan</h3>
                        <p class="text-gray-600">
                            @if(request('search') || request('status') || request('tahun'))
                                Tidak ditemukan keputusan dengan filter yang dipilih.
                            @else
                                Belum ada keputusan yang tersedia saat ini.
                            @endif
                        </p>
                    </div>
                </div>
        ...
        <div class="mt-8">
            <nav class="flex items-center justify-between">
                <div class="flex items-center text-sm text-gray-700">
                    Menampilkan {{ $keputusan->firstItem() ?? 0 }} - {{ $keputusan->lastItem() ?? 0 }} dari {{ $keputusan->total() }} keputusan
                </div>
                <div class="flex items-center space-x-2">
                    @if($keputusan->onFirstPage())
                    ...
                    @else
                        <a href="{{ $keputusan->previousPageUrl() }}" class="px-3 py-2 text-sm font-medium text-black bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                            </svg>
                        </a>
                    ...
                    <div class="flex items-center space-x-1">
                        @foreach($keputusan->getUrlRange(1, $keputusan->lastPage()) as $page => $url)
                            @if($page == $keputusan->currentPage())
                                <span class="px-3 py-2 text-sm font-medium bg-orange-600 text-white rounded-lg shadow-md">{{ $page }}</span>
                            @else
                    ...
                    @if($keputusan->hasMorePages())
                        <a href="{{ $keputusan->nextPageUrl() }}" class="px-3 py-2 text-sm font-medium text-black bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<!-- Keputusan Detail Modal -->
<div id="keputusanModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-hidden">
            <div class="flex items-center justify-between p-6 border-b">
                <h3 id="modalTitle" class="text-xl font-semibold text-gray-900">Detail Keputusan</h3>
...
@push('scripts')
<script>
// Show keputusan detail modal
function showKeputusanDetail(id, judul, konten, tanggalDokumen, penulis, nomorDokumen, namaInstansi, jenisLabel, statusLabel, thumbnail, lampiran) {
    // Set modal title
    ...
    let modalContent = `
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-2">
                    <button onclick="shareKeputusan('${escapeHtml(judul)}', '${window.location.origin}/dasar-hukum/keputusan/${id}')"
    ...
    if (lampiran && Array.isArray(lampiran) && lampiran.length > 0) {
        ...
        lampiran.forEach((file, index) => {
            ...
            modalContent += `
                <a href="/dasar-hukum/keputusan/${id}/download/${escapeHtml(filePath.split('/').pop())}"
                   target="_blank"
                   class="flex items-center justify-between p-3 bg-gray-50 rounded-lg hover:bg-gray-100 transition-colors group">
                    <div class="flex items-center">
                        ...
                        <div>
                            <p class="text-sm font-medium text-gray-900 group-hover:text-orange-600">${escapeHtml(fileName)}</p>
                        </div>
                    </div>
...
// Share functionality
function shareKeputusan(title, url) {
    if (navigator.share) {
        navigator.share({
            title: title,
            url: url
...
function printKeputusan() {
    ...
    printWindow.document.write(`
        <html>
            <head>
                <title>Keputusan - ${document.getElementById('modalTitle').textContent}</title>
                <style>
