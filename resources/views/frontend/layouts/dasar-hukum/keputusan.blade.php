    ...
    <div class="w-full px-2 sm:px-4 lg:px-6 py-6">
        ...
        <div class="mb-4 flex justify-end">
            <div class="relative w-96">
                <input type="text" id="searchInput" placeholder="Cari keputusan  .  .  .  .  .  .  .  . "
                       class="w-full px-2 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500 text-base transition-all duration-300 hover:border-purple-400 focus:border-purple-500 focus:ring-purple-500">
        ...
        <div id="loadingState" class="text-center py-12">
            ...
            <p class="mt-4 text-gray-600">Memuat keputusan...</p>
        </div>
        ...
        <div id="errorState" class="hidden text-center py-12">
            <div class="bg-red-50 border border-red-200 rounded-lg p-6 max-w-md mx-auto">
                ...
                <h3 class="text-lg font-medium text-red-800 mb-2">Gagal Memuat Keputusan</h3>
                <p class="text-red-600 mb-4">Terjadi kesalahan saat memuat data keputusan.</p>
                <button onclick="loadKeputusan()" class="bg-red-600 text-white px-4 py-2 rounded-lg hover:bg-red-700 transition-colors">
                    Coba Lagi
                </button>
            </div>
        </div>
        ...
        <div id="emptyState" class="hidden text-center py-12">
            <div class="bg-gray-50 border border-gray-200 rounded-lg p-6 max-w-md mx-auto">
                ...
                <p class="text-gray-600">Belum ada keputusan yang tersedia saat ini.</p>
            </div>
        </div>

        <!-- Keputusan Grid -->
        <div id="keputusanGrid" class="hidden grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 sm:gap-4 transition-all duration-500">
            <!-- Keputusan cards will be populated here -->
        </div>
        ...
        <div id="paginationContainer" class="hidden mt-6">
            <nav class="flex items-center justify-between">
                ...
                <div class="text-sm text-gray-700">
                    Menampilkan <span id="showingFrom">0</span> - <span id="showingTo">0</span> dari <span id="totalItems">0</span> keputusan
                </div>
            </nav>
        </div>
    </div>
...
// Load keputusan data
async function loadKeputusan(page = 1, isSearch = false, isPagination = false) {
    try {
        ...
        if (data.success) {
            ...
            if (isSearch || isPagination) {
                displayKeputusan(data.data);
                updatePagination(data.pagination);

                // Show elements immediately for smooth experience
                const gridElement = document.getElementById('keputusanGrid');
...
// Display keputusan cards
function displayKeputusan(keputusan) {
    const grid = document.getElementById('keputusanGrid');
    ...
    grid.innerHTML = keputusan.map((item, index) => `
        <article class="bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-pointer transform hover:scale-105 animate-fade-in" style="animation-delay: ${index * 100}ms" onclick="showKeputusanDetail(${item.id})">
            <div class="relative">
                ${item.thumbnail ? `
                ...
                ` : `
                    <div class="w-full h-32 bg-gradient-to-br from-purple-400 to-indigo-500 flex items-center justify-center transition-transform duration-300 hover:scale-110">
            ...
            <div class="p-3">
                ...
                <h3 class="text-sm font-semibold text-gray-900 mb-2 line-clamp-2 hover:text-purple-600 transition-colors duration-300">
                    ${item.judul}
                </h3>
                ...
                ${item.tags && item.tags.length > 0 ? `
                    <div class="mt-4 flex flex-wrap gap-2">
                        ${item.tags.map(tag => `
                            <span class="bg-gray-100 text-gray-700 text-xs px-2 py-1 rounded-full hover:bg-purple-100 hover:text-purple-700 transition-colors duration-300">
                                ${tag}
                            </span>
                        `).join('')}
                    </div>
...
async function showKeputusanDetail(id) {
    try {
                ...
                <div class="space-y-6">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        ...
                        <div>
                            ...
                            <p class="text-gray-600">${item.jenis_label || 'Keputusan'}</p>
                        </div>
                        <div>
        ...
        console.error('Error loading keputusan detail:', error);
    }
}
...
function updatePagination(pagination) {
    ...
    for (let i = startPage; i <= endPage; i++) {
        ...
        pageBtn.onclick = () => loadKeputusan(i, false, true);
    ...
    prevBtn.onclick = () => currentPage > 1 && loadKeputusan(currentPage - 1, false, true);
    nextBtn.onclick = () => currentPage < totalPages && loadKeputusan(currentPage + 1, false, true);
...
function showLoading() {
    ...
    document.getElementById('keputusanGrid').classList.add('hidden');
...
document.getElementById('searchInput').addEventListener('input', debounce((e) => {
    ...
    // If search is empty, load all keputusan immediately
    if (currentSearch === '') {
        loadKeputusan(1, true);
    } else {
...
#pageNumbers button.bg-purple-600 {
