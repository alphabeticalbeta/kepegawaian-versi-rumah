...
@section('title', 'Surat Rektor - Universitas Mulawarman')
...
<div class="min-h-screen bg-gray-50">
        ...
        <div class="relative px-6 py-4 sm:px-8 sm:py-20">
            <div class="mx-auto max-w-4xl text-center">
                ...
                <h1 class="text-4xl font-bold tracking-tight text-black sm:text-5xl mb-4">
                    Surat Rektor Terkini
                </h1>
                <p class="text-base text-gray-600 sm:text-lg">
                    Daftar Surat Rektor Universitas Mulawarman
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="w-full px-2 sm:px-4 lg:px-6 py-6">
        ...
        <div class="mb-6 bg-white rounded-2xl shadow-xl p-4 transition-all duration-300 hover:shadow-2xl">
            <form method="GET" action="{{ route('dasar-hukum.surat-rektor-unmul.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                ...
                <div>
                    ...
                    <select name="tahun" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all duration-200 hover:border-indigo-400">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari judul/nomor dokumen..."
                               class="w-full px-3 py-2 pr-10 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all duration-200 hover:border-indigo-400"
                               id="searchInput">
                        <!-- Loading spinner -->
                        ...
                        <!-- Clear button -->
                        @if(request('search'))
                            <a href="{{ route('dasar-hukum.surat-rektor-unmul.index') }}" class="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors" title="Hapus pencarian" id="clearSearch">
        ...
        <div class="mb-4 flex flex-wrap gap-2">
            ...
            @if(request('search'))
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-indigo-100 text-indigo-800">
                    ...
                    <a href="{{ route('dasar-hukum.surat-rektor-unmul.index', array_merge(request()->except('search'))) }}" class="ml-2 text-indigo-600 hover:text-indigo-800">
            ...
            @if(request('status'))
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                    ...
                    <a href="{{ route('dasar-hukum.surat-rektor-unmul.index', array_merge(request()->except('status'))) }}" class="ml-2 text-blue-600 hover:text-blue-800">
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </a>
            ...
            @if(request('tahun'))
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                    Tahun: {{ request('tahun') }}
                    <a href="{{ route('dasar-hukum.surat-rektor-unmul.index', array_merge(request()->except('tahun'))) }}" class="ml-2 text-green-600 hover:text-green-800">
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </a>
        ...
        <!-- Surat Rektor Universitas Mulawarman Grid - Hampir Penuh -->
        <div class="w-full">
            <div id="surat_rektorGrid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 transition-all duration-500">
                @forelse($surat_rektor_unmul as $index => $item)
                <article class="bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-pointer transform hover:scale-105 animate-fade-in"
                         ...
                         onclick="showSuratRektorUnmulDetail({{ $item->id }}, '{{ addslashes($item->judul) }}', '{{ addslashes($item->konten) }}', '{{ $item->tanggal_dokumen }}', '{{ addslashes($item->penulis ?? 'Admin') }}', '{{ addslashes($item->nomor_dokumen ?? '-') }}', '{{ addslashes($item->nama_instansi ?? '-') }}', '{{ addslashes($item->jenis_label ?? 'surat_rektor') }}', '{{ addslashes($item->status_label ?? '-') }}', '{{ $item->thumbnail }}', {{ json_encode($item->lampiran ?? []) }})">
                ...
                <div class="col-span-full text-center py-12">
                    <div class="bg-gray-50 border border-gray-200 rounded-lg p-6 max-w-md mx-auto">
                        ...
                        <p class="text-gray-600">
                            @if(request('search') || request('status') || request('tahun'))
                                Tidak ditemukan Surat Rektor dengan filter yang dipilih.
                            @else
                                Belum ada Surat Rektor yang tersedia saat ini.
                            @endif
                        </p>
                    </div>
                </div>
        ...
        <!-- Pagination -->
        @if($surat_rektor_unmul->hasPages())
        <div class="mt-8">
            <nav class="flex items-center justify-between">
                <div class="flex items-center text-sm text-gray-700">
                    Menampilkan {{ $surat_rektor_unmul->firstItem() ?? 0 }} - {{ $surat_rektor_unmul->lastItem() ?? 0 }} dari {{ $surat_rektor_unmul->total() }} surat Rektor Universitas Mulawarman
                </div>
                <div class="flex items-center space-x-2">
                    @if($surat_rektor_unmul->onFirstPage())
                    ...
                    @else
                        <a href="{{ $surat_rektor_unmul->previousPageUrl() }}" class="px-3 py-2 text-sm font-medium text-black bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                            </svg>
                        </a>
                    ...
                    <div class="flex items-center space-x-1">
                        @foreach($surat_rektor_unmul->getUrlRange(1, $surat_rektor_unmul->lastPage()) as $page => $url)
                            @if($page == $surat_rektor_unmul->currentPage())
                    ...
                    @if($surat_rektor_unmul->hasMorePages())
                        <a href="{{ $surat_rektor_unmul->nextPageUrl() }}" class="px-3 py-2 text-sm font-medium text-black bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<!-- Surat Rektor Universitas Mulawarman Detail Modal -->
...
@push('scripts')
<script>
// Show Surat Rektor Universitas Mulawarman detail modal
function showSuratRektorUnmulDetail(id, judul, konten, tanggalDokumen, penulis, nomorDokumen, namaInstansi, jenisLabel, statusLabel, thumbnail, lampiran) {
    // Set modal title
    ...
    let modalContent = `
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-2">
                    <button onclick="shareSuratRektorUnmul('${escapeHtml(judul)}', '${window.location.origin}/dasar-hukum/surat-rektor-unmul/${id}')"
                    ...
                    <button onclick="printSuratRektorUnmul()"
    ...
    if (lampiran && Array.isArray(lampiran) && lampiran.length > 0) {
        ...
        lampiran.forEach((file, index) => {
            ...
            modalContent += `
                <a href="/dasar-hukum/surat-rektor-unmul/${id}/download/${escapeHtml(filePath.split('/').pop())}"
                   target="_blank"
...
// Share functionality
function shareSuratRektorUnmul(title, url) {
    if (navigator.share) {
        navigator.share({
            title: title,
            url: url
...
// Print functionality
function printSuratRektorUnmul() {
    const modalContent = document.getElementById('modalContent').innerHTML;
    const printWindow = window.open('', '_blank');
    printWindow.document.write(`
        <html>
            <head>
                <title>surat_rektor - ${document.getElementById('modalTitle').textContent}</title>
                <style>
