...
@section('title', 'Surat Edaran - Universitas Mulawarman')
...
<div class="min-h-screen bg-gray-50">
        ...
        <div class="relative px-6 py-4 sm:px-8 sm:py-20">
            <div class="mx-auto max-w-4xl text-center">
                ...
                <h1 class="text-4xl font-bold tracking-tight text-black sm:text-5xl mb-4">
                    Surat Edaran Terkini
                </h1>
                <p class="text-base text-gray-600 sm:text-lg">
                    Daftar Surat Edaran Universitas Mulawarman
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="w-full px-2 sm:px-4 lg:px-6 py-6">
        ...
        <div class="mb-6 bg-white rounded-2xl shadow-xl p-4 transition-all duration-300 hover:shadow-2xl">
            <form method="GET" action="{{ route('dasar-hukum.surat-edaran.index') }}" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Status</label>
                    <select name="status" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all duration-200 hover:border-purple-400">
                ...
                <div>
                    ...
                    <select name="tahun" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all duration-200 hover:border-purple-400">
                ...
                <div>
                    ...
                    <div class="relative">
                        <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari judul/nomor dokumen..."
                               class="w-full px-3 py-2 pr-10 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500 transition-all duration-200 hover:border-purple-400"
                               id="searchInput">
                        <!-- Loading spinner -->
                        ...
                        <!-- Clear button -->
                        @if(request('search'))
                            <a href="{{ route('dasar-hukum.surat-edaran.index') }}" class="absolute right-3 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors" title="Hapus pencarian" id="clearSearch">
        ...
        <div class="mb-4 flex flex-wrap gap-2">
            ...
            @if(request('search'))
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                    ...
                    <a href="{{ route('dasar-hukum.surat-edaran.index', array_merge(request()->except('search'))) }}" class="ml-2 text-purple-600 hover:text-purple-800">
            ...
            @if(request('status'))
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                    ...
                    <a href="{{ route('dasar-hukum.surat-edaran.index', array_merge(request()->except('status'))) }}" class="ml-2 text-blue-600 hover:text-blue-800">
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </a>
            ...
            @if(request('tahun'))
                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                    Tahun: {{ request('tahun') }}
                    <a href="{{ route('dasar-hukum.surat-edaran.index', array_merge(request()->except('tahun'))) }}" class="ml-2 text-green-600 hover:text-green-800">
                        <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"></path>
                        </svg>
                    </a>
        ...
        <!-- surat_edaran Grid - Hampir Penuh -->
        <div class="w-full">
            <div id="surat_edaranGrid" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 transition-all duration-500">
                @forelse($surat_edaran as $index => $item)
                <article class="bg-white rounded-lg shadow-sm overflow-hidden hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-pointer transform hover:scale-105 animate-fade-in"
                         ...
                         onclick="showSuratEdaranDetail({{ $item->id }}, '{{ addslashes($item->judul) }}', '{{ addslashes($item->konten) }}', '{{ $item->tanggal_dokumen }}', '{{ addslashes($item->penulis ?? 'Admin') }}', '{{ addslashes($item->nomor_dokumen ?? '-') }}', '{{ addslashes($item->nama_instansi ?? '-') }}', '{{ addslashes($item->jenis_label ?? 'surat_edaran') }}', '{{ addslashes($item->status_label ?? '-') }}', '{{ $item->thumbnail }}', {{ json_encode($item->lampiran ?? []) }})">
                ...
                <div class="col-span-full text-center py-12">
                    <div class="bg-gray-50 border border-gray-200 rounded-lg p-6 max-w-md mx-auto">
                        ...
                        <h3 class="text-lg font-medium text-gray-800 mb-2">Tidak Ada Surat Edaran</h3>
                        <p class="text-gray-600">
                            @if(request('search') || request('status') || request('tahun'))
                                Tidak ditemukan Surat Edaran dengan filter yang dipilih.
                            @else
                                Belum ada Surat Edaran yang tersedia saat ini.
                            @endif
                        </p>
                    </div>
                </div>
        ...
        <!-- Pagination -->
        @if($surat_edaran->hasPages())
        <div class="mt-8">
            <nav class="flex items-center justify-between">
                <div class="flex items-center text-sm text-gray-700">
                    Menampilkan {{ $surat_edaran->firstItem() ?? 0 }} - {{ $surat_edaran->lastItem() ?? 0 }} dari {{ $surat_edaran->total() }} surat_edaran
                </div>
                <div class="flex items-center space-x-2">
                    @if($surat_edaran->onFirstPage())
                    ...
                    @else
                        <a href="{{ $surat_edaran->previousPageUrl() }}" class="px-3 py-2 text-sm font-medium text-black bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                            </svg>
                        </a>
                    ...
                    <div class="flex items-center space-x-1">
                        @foreach($surat_edaran->getUrlRange(1, $surat_edaran->lastPage()) as $page => $url)
                            @if($page == $surat_edaran->currentPage())
                    ...
                    @if($surat_edaran->hasMorePages())
                        <a href="{{ $surat_edaran->nextPageUrl() }}" class="px-3 py-2 text-sm font-medium text-black bg-white border border-gray-300 rounded-lg hover:bg-gray-50 transition-colors">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<!-- surat_edaran Detail Modal -->
<div id="suratEdaranModal" class="fixed inset-0 bg-black bg-opacity-50 hidden z-50 overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen p-4">
        <div class="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-hidden">
            <div class="flex items-center justify-between p-6 border-b">
                <h3 id="modalTitle" class="text-xl font-semibold text-gray-900">Detail surat_edaran</h3>
...
@push('scripts')
<script>
// Show surat_edaran detail modal
function showSuratEdaranDetail(id, judul, konten, tanggalDokumen, penulis, nomorDokumen, namaInstansi, jenisLabel, statusLabel, thumbnail, lampiran) {
    // Set modal title
    ...
    let modalContent = `
        <div class="space-y-6">
            <div class="flex items-center justify-between">
                ...
                <div class="flex items-center space-x-2">
                    <button onclick="shareSuratEdaran('${escapeHtml(judul)}', '${window.location.origin}/dasar-hukum/surat-edaran/${id}')"
                    ...
                    <button onclick="printSuratEdaran()"
    ...
    if (lampiran && Array.isArray(lampiran) && lampiran.length > 0) {
        ...
        lampiran.forEach((file, index) => {
            ...
            modalContent += `
                <a href="/dasar-hukum/surat-edaran/${id}/download/${escapeHtml(filePath.split('/').pop())}"
                   target="_blank"
    ...
    document.getElementById('suratEdaranModal').classList.remove('hidden');
...
// Close modal
function closeModal() {
    document.getElementById('suratEdaranModal').classList.add('hidden');
...
// Share functionality
function shareSuratEdaran(title, url) {
    if (navigator.share) {
        navigator.share({
            title: title,
            url: url
...
// Print functionality
function printSuratEdaran() {
    const modalContent = document.getElementById('modalContent').innerHTML;
    const printWindow = window.open('', '_blank');
    printWindow.document.write(`
        <html>
            <head>
                <title>surat_edaran - ${document.getElementById('modalTitle').textContent}</title>
                <style>
