...
@section('title', 'Validasi Usulan NUPTK: ' . $usulan->pegawai->nama_lengkap)
...
<div class="min-h-screen bg-gradient-to-br from-slate-50 via-white to-indigo-50/30">
            ...
            <div class="py-6 flex flex-wrap gap-4 justify-between items-center">
                <div>
                    <h1 class="text-2xl font-bold text-gray-900">
                        Validasi Usulan NUPTK
                    </h1>
                    <p class="mt-1 text-sm text-gray-500">
                        Validasi field-by-field usulan NUPTK untuk {{ $usulan->pegawai->nama_lengkap }}
                    </p>
                </div>
    ...
    <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden mb-6">
            ...
            <div class="p-6">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        <label class="block text-sm font-semibold text-gray-800">NUPTK Saat Ini</label>
                        <p class="text-xs text-gray-600 mb-2">NUPTK yang sudah dimiliki</p>
                        <input type="text" value="{{ $usulan->pegawai->nuptk ?? '-' }}"
                               class="block w-full border-gray-200 rounded-lg shadow-sm bg-gray-100 px-4 py-3 text-gray-800 font-medium cursor-not-allowed" disabled>
                    </div>
                </div>
            </div>
        ...
        @include('backend.layouts.views.shared.usul-nuptk.usulan-detail-validation-table', ['isViewOnly' => $isViewOnly])
    </div>
</div>
...
@push('scripts')
<script>
// Standardized submitAction function for NUPTK validation
...
function processStatusChangeRequest(newStatus) {
    fetch(`{{ route('backend.kepegawaian-universitas.usulan.change-status', $usulan->id) }}`, {
        ...
        body: JSON.stringify({
            ...
            keterangan: 'Status diubah melalui halaman validasi NUPTK'
        })
    })
    .then(response => response.json())
    .then(data => {
