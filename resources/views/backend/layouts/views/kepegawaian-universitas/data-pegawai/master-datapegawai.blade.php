...
<div class="container mx-auto px-4 sm:px-6 lg:px-8 py-8">
    ...
    <div class="bg-white p-6 rounded-2xl shadow-lg border border-gray-100 mb-8">
        ...
        <div class="pt-5">
            <form method="GET" action="{{ route('backend.kepegawaian-universitas.data-pegawai.index') }}">
                    ...
                    <tr class="hover:bg-slate-50/50 transition-colors">
                        ...
                        <td class="px-6 py-4 text-center align-middle">
                            <div class="flex justify-center items-center gap-2">
                                {{-- Tombol Lihat (Read) --}}
                                <a href="{{ route('backend.kepegawaian-universitas.data-pegawai.show', $pegawai) }}" class="p-2 text-slate-500 hover:text-green-600 hover:bg-green-100 rounded-full transition-colors" title="Lihat Detail">
                                    <i data-lucide="eye" class="w-4 h-4"></i>
                                </a>
                                {{-- Tombol Edit (Update) --}}
                                <a href="{{ route('backend.kepegawaian-universitas.data-pegawai.edit', $pegawai) }}" class="p-2 text-slate-500 hover:text-indigo-600 hover:bg-indigo-100 rounded-full transition-colors" title="Edit Data">
                                    <i data-lucide="edit" class="w-4 h-4"></i>
                                </a>
                                {{-- Tombol Hapus (Delete) --}}
                                <form action="{{ route('backend.kepegawaian-universitas.data-pegawai.destroy', $pegawai) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data pegawai ini?');">
                                    @csrf
                                    @method('DELETE')
