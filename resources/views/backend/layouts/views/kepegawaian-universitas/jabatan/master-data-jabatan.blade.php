        ...
        <div class="relative px-4 py-8 sm:px-6 sm:py-10">
            <div class="mx-auto max-w-full text-center">
                ...
                <p class="text-base text-black sm:text-lg">
                    Kelola hirarki jabatan untuk sistem usulan kepegawaian
                </p>
            </div>
        </div>
    </div>
    ...
    <div class="relative z-10 -mt-8 px-4 sm:px-6 lg:px-8">
        ...
        <div class="mb-4 bg-white rounded-2xl shadow-xl p-4 transition-all duration-300 hover:shadow-2xl">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-3">
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Jenis Pegawai</label>
                    <select id="filterJenisPegawai" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all duration-200 hover:border-indigo-400">
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-2">Jenis Jabatan</label>
                    <select id="filterJenisJabatan" class="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 transition-all duration-200 hover:border-indigo-400">
                        <option value="">Semua</option>
                ...
                <table class="w-full divide-y divide-gray-200">
                    <thead class="bg-white">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-xs font-bold text-black text-center uppercase tracking-wider w-32">Jenis Pegawai</th>
                            <th class="px-4 py-3 text-xs font-bold text-black text-center uppercase tracking-wider">Jabatan</th>
                            <th class="px-4 py-3 text-xs font-bold text-black text-center uppercase tracking-wider">Jenis Jabatan</th>
                            <th class="px-4 py-3 text-xs font-bold text-black text-center uppercase tracking-wider w-20">Level</th>
                            <th class="px-4 py-3 text-xs font-bold text-black text-center tracking-wider w-40">Aksi</th>
                        </tr>
                    </thead>
                    <tbody id="jabatanTableBody" class="bg-white divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50 transition-colors">
                            ...
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900">
                                <div class="font-medium">{{ $jabatan->jabatan }}</div>
                            </td>

                            <!-- Jenis Jabatan -->
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900 text-center">
                                <div class="font-medium">{{ $jabatan->jenis_jabatan }}</div>
                            </td>

                            <!-- Level -->
                            <td class="px-4 py-3 whitespace-nowrap text-sm text-gray-900 text-center">
                                <div class="font-medium">{{ $jabatan->hierarchy_level ?? '-' }}</div>
                            </td>
                            ...
                            <td class="px-4 py-3 whitespace-nowrap text-sm font-medium text-center">
                                <div class="flex justify-center items-center gap-2">
                                    ...
                                    <form action="{{ route('backend.kepegawaian-universitas.jabatan.destroy', $jabatan) }}" method="POST" onsubmit="return confirm('Apakah Anda yakin ingin menghapus data jabatan ini?');" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
                        ...
                        <tr>
                            <td colspan="6" class="px-4 py-12 text-center">
                                ...
                                <div class="flex flex-col items-center">
                                    ...
                                    <h3 class="text-lg font-medium text-gray-900">Tidak ada data jabatan</h3>
                                    <p class="text-gray-500 mt-1">Tambah data jabatan untuk memulai</p>
                                </div>
                            </td>
                        </tr>
                        @endforelse
        ...
        <div id="paginationContainer" class="bg-white rounded-2xl shadow-xl overflow-hidden mt-6">
            <div class="px-4 py-4 flex items-center justify-between">
                <div class="flex items-center text-sm text-gray-700">
                    <span id="paginationInfo">Menampilkan {{ $jabatans->firstItem() ?? 0 }} - {{ $jabatans->lastItem() ?? 0 }} dari {{ $jabatans->total() }} data</span>
                </div>
                <div class="flex items-center space-x-2">
                    ...
                    @else
                        <a href="{{ $jabatans->previousPageUrl() }}" class="px-3 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-md hover:bg-gray-50 transition-colors duration-200">
                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path>
                            </svg>
                        </a>
                    ...
                    @if($jabatans->hasMorePages())
                        <a href="{{ $jabatans->nextPageUrl() }}" class="px-3 py-2 text-sm font-medium text-gray-500 bg-white border border-gray-300 rounded-md hover:bg-gray-50 transition-colors duration-200">
                            <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
