...
@section('title', 'Tambah Periode Usulan - Admin Universitas')
...
<div class="min-h-screen bg-gradient-to-br from-indigo-50 via-purple-50 to-blue-50">
    ...
    <div class="admin-universitas-bg text-white p-6 rounded-2xl mb-6 shadow-2xl">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-2xl font-bold mb-2">Tambah Periode Usulan</h1>
                <p class="text-indigo-100">Buat periode baru untuk pembukaan usulan kepegawaian</p>
            </div>
    ...
    <div class="bg-white/90 backdrop-blur-xl rounded-2xl shadow-xl border border-white/30 p-8">
        <form action="{{ route('admin-universitas.periode-usulan.store') }}" method="POST" class="space-y-6">
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <select name="jenis_usulan"
                        ...
                        <option value="Jabatan Fungsional" {{ old('jenis_usulan') == 'Jabatan Fungsional' ? 'selected' : '' }}>Jabatan Fungsional</option>
                        <option value="Jabatan Struktural" {{ old('jenis_usulan') == 'Jabatan Struktural' ? 'selected' : '' }}>Jabatan Struktural</option>
                        <option value="Kenaikan Pangkat" {{ old('jenis_usulan') == 'Kenaikan Pangkat' ? 'selected' : '' }}>Kenaikan Pangkat</option>
                        <option value="Tugas Belajar" {{ old('jenis_usulan') == 'Tugas Belajar' ? 'selected' : '' }}>Tugas Belajar</option>
                        <option value="Umum" {{ old('jenis_usulan') == 'Umum' ? 'selected' : '' }}>Umum</option>
                    </select>
                    @error('jenis_usulan')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="number"
                           name="tahun_periode"
                           id="tahun_periode"
                           value="{{ old('tahun_periode', date('Y')) }}"
                           min="2020"
                           max="2050"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <input type="date"
                               ...
                               value="{{ old('tanggal_mulai_perbaikan') }}"
                    ...
                    <div>
                        ...
                        <input type="date"
                               ...
                               value="{{ old('tanggal_selesai_perbaikan') }}"
