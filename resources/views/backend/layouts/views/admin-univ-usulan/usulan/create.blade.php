...
@section('title', 'Buat ' . $namaUsulan . ' - Admin Universitas Usulan')
    ...
    <div class="bg-gradient-to-r from-blue-600 to-indigo-600 text-white p-6 rounded-2xl mb-6 shadow-2xl">
        <div class="flex items-center justify-between">
            ...
            <div class="hidden md:block">
                <a href="{{ route('backend.admin-univ-usulan.usulan.index', ['jenis' => $jenisUsulan]) }}"
    ...
    <div class="bg-white/90 backdrop-blur-xl rounded-2xl shadow-xl border border-white/30 p-8">
        @if($periode->status === 'Buka')
            <form action="#" method="POST" enctype="multipart/form-data" class="space-y-6">
                ...
                @switch($jenisUsulan)
                    @case('jabatan')
                        @include('backend.layouts.views.admin-univ-usulan.usulan.forms.jabatan')
                        @break
                    @case('kepangkatan')
                        @include('backend.layouts.views.admin-univ-usulan.usulan.forms.kepangkatan')
                        @break
                    @case('tugas-belajar')
                        @include('backend.layouts.views.admin-univ-usulan.usulan.forms.tugas-belajar')
                        @break
                    @default
                        @include('backend.layouts.views.admin-univ-usulan.usulan.forms.default')
                @endswitch
