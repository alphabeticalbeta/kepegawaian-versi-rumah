...
<div class="container mx-auto px-4 sm:px-6 lg:px-8">
    <div class="card">
        ...
        <div class="card-header text-center">
            <h1 class="text-2xl font-semibold text-gray-900">Master Data Sub Unit Kerja</h1>
            <div class="mt-4">
                <a href="{{ route('backend.admin-univ-usulan.sub-unitkerja.create') }}"
        ...
        <div class="card-body p-0">
            <table class="w-full text-sm text-gray-700 border-collapse">
                ...
                <tbody>
                    @forelse($subUnitKerjas as $index => $subUnitKerja)
                        <tr>
                            <td class="px-4 py-2 text-center">{{ $index + $subUnitKerjas->firstItem() }}</td>
                            <td class="px-4 py-2 font-medium text-current">{{ $subUnitKerja->unitKerja->nama }}</td>
                            <td class="px-4 py-2 font-medium text-current">{{ $subUnitKerja->nama }}</td>
                            <td class="px-4 py-2">
                                <div class="flex justify-center items-center space-x-2">
                                    {{-- Edit --}}
                                    <a href="{{ route('backend.admin-univ-usulan.sub-unitkerja.edit', $subUnitKerja) }}"
                                    ...
                                    {{-- Hapus --}}
                                    <form action="{{ route('backend.admin-univ-usulan.sub-unitkerja.destroy', $subUnitKerja) }}"
                                          method="POST"
