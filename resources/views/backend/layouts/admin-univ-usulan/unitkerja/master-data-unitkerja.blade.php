            ...
            <table class="w-full text-sm text-gray-700 border-collapse">
                <thead class="bg-gray-100 uppercase text-xs font-semibold">
                    <tr>
                        ...
                        <th class="px-4 py-2">Nama Unit Kerja</th>
                ...
                <tbody>
                    @forelse($unitkerjas as $index => $unitkerja)
                        <tr>
                            <td class="px-4 py-2 text-center">{{ $index + $unitkerjas->firstItem() }}</td>
                            <td class="px-4 py-2 font-medium text-current">{{ $unitkerja->nama }}</td>
                            <td class="px-4 py-2">
                                <div class="flex justify-center items-center space-x-2">
                                    {{-- Edit --}}
                                    <a href="{{ route('backend.admin-univ-usulan.unitkerja.edit', $unitkerja) }}"
                                    ...
                                    {{-- Hapus --}}
                                    <form action="{{ route('backend.admin-univ-usulan.unitkerja.destroy', $unitkerja) }}"
                                          method="POST"
