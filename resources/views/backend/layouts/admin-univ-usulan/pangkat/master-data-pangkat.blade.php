...
<div class="container mx-auto px-4 sm:px-6 lg:px-8">
    <div class="card">
        ...
        <div class="card-header text-center">
            <h1 class="text-2xl font-semibold text-gray-900">Master Data Pangkat</h1>
            <div class="mt-4">
            ...
            <table class="w-full text-sm text-gray-700 border-collapse">
                <thead class="bg-gray-100 uppercase text-xs font-semibold">
                    <tr>
                        ...
                        <th class="px-4 py-2 text-center">Pangkat</th>
                ...
                <tbody>
                    @forelse($pangkats as $index => $pangkat)
                        <tr>
                            <td class="px-4 py-2 text-center">{{ $index + $pangkats->firstItem() }}</td>
                            <td class="px-4 py-2 text-center font-medium">{{ $pangkat->pangkat }}</td>
                            <td class="px-4 py-2 text-center">
                                <div class="flex justify-center items-center space-x-2">
                                    ...
                                    <form action="{{ route('backend.admin-univ-usulan.pangkat.destroy', $pangkat) }}"
                                          method="POST"
                                          onsubmit="return confirm('Apakah Anda yakin ingin menghapus pangkat ini?');">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit"
