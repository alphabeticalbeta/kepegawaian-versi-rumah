                ...
                <div class="xl:col-span-1 space-y-8">
                    ...
                    <div class="bg-white p-6 rounded-2xl shadow-lg shadow-slate-600/5 border border-slate-100 text-center">
                        ...
                        <div class="relative inline-block mb-4">
                            <img src="{{ $pegawai->foto ? asset('storage/' . $pegawai->foto) : 'https://ui-avatars.com/api/?name=' . urlencode($pegawai->nama_lengkap) }}"
                                alt="Foto Profil"
                                class="w-32 h-32 object-cover rounded-full mx-auto border-4 border-white shadow-lg ring-2 ring-slate-100">

                            @if($isEditing)
                                <!-- Overlay for edit mode -->
                                <div class="absolute inset-0 bg-black bg-opacity-40 rounded-full flex items-center justify-center opacity-0 hover:opacity-100 transition-opacity duration-200">
                                ...
                                <!-- Clickable area covering the entire image -->
                                <label for="foto" class="absolute inset-0 cursor-pointer rounded-full" title="Klik untuk mengganti foto"></label>
                            @endif
                        </div>
                        ...
                        @if($isEditing)
                            ...
                            <div class="mb-4">
                                <label for="foto" class="inline-flex items-center px-4 py-2 bg-indigo-50 hover:bg-indigo-100 text-indigo-600 text-sm font-medium rounded-lg cursor-pointer transition-colors duration-200 border border-indigo-200">
                        ...
                        <div>
                            <h3 class="text-xl font-bold text-slate-800 mb-2">
                                @if($pegawai->gelar_depan){{ $pegawai->gelar_depan }} @endif{{ $pegawai->nama_lengkap }}@if($pegawai->gelar_belakang) {{ $pegawai->gelar_belakang }}@endif
                            </h3>
                            <p class="text-sm text-slate-500 mb-1">{{ $pegawai->nip }}</p>
                            <p class="text-sm text-indigo-600 font-medium">{{ $pegawai->email }}</p>
                        </div>
                    </div>
