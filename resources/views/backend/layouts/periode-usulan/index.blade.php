...
document.addEventListener('DOMContentLoaded', function() {
    const jenisUsulanSelect = document.getElementById('jenisUsulanSelect');
    const tambahPeriodeBtn = document.getElementById('tambahPeriodeBtn');
    const filterJenis = document.getElementById('filterJenis');
    const tableRows = document.querySelectorAll('tbody tr[data-jenis]');

    // Enable/disable button berdasarkan pilihan jenis usulan
    jenisUsulanSelect.addEventListener('change', function() {
        const selectedValue = this.value;

        if (selectedValue) {
            tambahPeriodeBtn.disabled = false;
            tambahPeriodeBtn.classList.remove('bg-gray-400', 'cursor-not-allowed');
            tambahPeriodeBtn.classList.add('bg-indigo-600', 'hover:bg-indigo-700', 'active:bg-indigo-900', 'focus:outline-none', 'focus:border-indigo-900', 'focus:ring', 'ring-indigo-300', 'transition', 'ease-in-out', 'duration-150');

            // Update href untuk redirect ke form yang sesuai
            tambahPeriodeBtn.onclick = function() {
                window.location.href = `{{ route('backend.admin-univ-usulan.periode-usulan.create') }}?jenis=${selectedValue}`;
            };
        } else {
            tambahPeriodeBtn.disabled = true;
            tambahPeriodeBtn.classList.add('bg-gray-400', 'cursor-not-allowed');
            tambahPeriodeBtn.classList.remove('bg-indigo-600', 'hover:bg-indigo-700', 'active:bg-indigo-900', 'focus:outline-none', 'focus:border-indigo-900', 'focus:ring', 'ring-indigo-300', 'transition', 'ease-in-out', 'duration-150');
            tambahPeriodeBtn.onclick = null;
        }
    });

    // Filter tabel berdasarkan jenis usulan
    filterJenis.addEventListener('change', function() {
        ...
        tableRows.forEach(row => {
            ...
            if (!filterValue || rowJenis === filterValue) {
                row.style.display = '';
            } else {
